""" SecureConfig base class for simplifying load of encrypted config files
    (default: serialized dict).

Features:

* Instantiate with either a file path or a string of text
  (or none for a blank slate).
* Stores simple "config" dictionary in .cfg
* ConfigParser-like interface (get, set, add_section, remove_section, write)
* .write(filehandle) method serializes dictionary, encrypts, and writes to
  open filehandle
...
from .cryptkeeper import CryptKeeperAccessMethods
from .exceptions import ReadOnlyConfigError, SecureConfigException


class SecureConfig(CryptKeeperAccessMethods):
    """ Builds a SecureConfig object.

    Without any arguments, SecureConfig.__init__ is a blank slate into which
    you can load dictionary-based data directly into cfg.

    To decrypt data, SecureConfig requires a CryptKeeper object, but the
    following class methods help set this up for you:
    ...
    If ck==None, SecureConfig will attempt to read the file as if it were
    stored exclusively in plaintext and throws SecureConfigException if it
    cannot be parsed.
    ...
    def __init__(self, filepath='', rawtxt='', readonly=False, **kwargs):
        ...
        if filepath and rawtxt:
            raise SecureConfigException(
                'Supply either filepath or rawtxt (not both).')
        ...
        if self.ck and rawtxt:
            ...
            try:
                ...
                # This approach may only work for SecureJson, which so far is
                # the only use for this base class. Please rework as needed to
                # better generalize.
                raise SecureConfigException(
                    'bad data or missing encryption key')
        else:
    ...
    def _read(self, filepath):  # pylint: disable=no-self-use
    ...
    def get(self, section, param):
        """ Provides ConfigParser-like interface retrieve variables
            from sections.

        If your config data is shallow, i.e. non-hierarchical, either a
        TypeError or a NameError will be thrown (depending on your data).

        :param section:  top-level "section" of configuration.
        :param param:    parameter within "section" whose value will be
        returned.
        """
    ...
    def write(self, fh=None):  # pylint: disable=invalid-name
        """ If .readonly=False, serializes, encrypts (if key) writing to
